...
import { resolvePlacementIssue } from '@/lib/server/deal-actions'
...
const RESOLVE_ISSUE_BODY_KEYS = new Set(['placementIndex', 'proofUrl'])
...
export async function POST(
  ...
  try {
    ...
    for (const key of Object.keys(body as Record<string, unknown>)) {
      if (!RESOLVE_ISSUE_BODY_KEYS.has(key)) {
    ...
    const bundle = await resolvePlacementIssue(dealId, session.user.id, placementIndex, proofUrl)
